...
for f_i = 1:length(all_files)
    ...
    for j = 1:length(time)
        ...
        %clf
        %plot(all_chemical_positions{f_i}(1), all_chemical_positions{f_i}(2), 'd')
        %hold on
        %plot(robot_position(1:j,1), robot_position(1:j,2))
        %plot(robot_position(j,1), robot_position(j,2),'.r')
        %disp('-----------------------------')
        %disp(angle/pi*180)
        %disp(q2angle(robot_pose(j,:)))
        %pause
    end
    ...
    all_resistances = [all_resistances; sensor_measurements(:)];
    ...
    for j = 1:length(time)
        ...
        summary_mus = [summary_mus; mean(sensor_measurements(j,valid_data))];
...
if 1
    ...
    all_resistances = [all_resistances; sensor2];
